...
async def delete_document(db: DbSession, tenant_id: int, document_id: int):
  tenant = (
  ...
  file_path: str | None = str(document.file_path)  

  if file_path is not None:
    try:
      response = settings.s3_client.delete_object(
        Bucket=settings.S3_BUCKET_NAME,
        Key=file_path,
      )
      print(f"DELETED FILE: s3://{settings.S3_BUCKET_NAME}/{file_path}")
      ...
      print(f"Unexpected S3 delete error: {e}")
